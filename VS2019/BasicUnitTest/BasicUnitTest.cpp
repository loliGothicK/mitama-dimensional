...
{
	REQUIRE(!std::is_constructible_v<meter_t, TestType>);
}
...
{
	REQUIRE(!std::is_constructible_v<ampere_t, TestType>);
}
...
{
	REQUIRE(!std::is_constructible_v<candela_t, TestType>);
}
...
{
	REQUIRE(!std::is_constructible_v<kelvin_t, TestType>);
}
...
{
	REQUIRE(!std::is_constructible_v<kilogram_t, TestType>);
}
...
{
	REQUIRE(!std::is_constructible_v<mol_t, TestType>);
}
...
{
	REQUIRE(!std::is_constructible_v<second_t, TestType>);
}
	...
	second_t, meter_t, ampere_t, candela_t, kelvin_t, kilogram_t, mol_t)
{
	...
	auto v = refined_for<sym::L<1>>(1 | meters);
}
