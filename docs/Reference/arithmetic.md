...
!!! Note
    This operator shall not participates in overload resolution unless `std::conjunction_v<is_same_dimensional_v<std::decay_t<L>, std::decay_t<R>>, is_addable<typename std::decay_t<L>::value_type, typename std::decay_t<R>::value_type>>` is `true`.
