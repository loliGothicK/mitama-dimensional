...
TEST_CASE("electric_displacement_field type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::electric_displacement_field)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<-2>*seconds*ampere<>)>>>);
}
