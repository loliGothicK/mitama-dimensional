...
TEST_CASE("compressibility type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::compressibility)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meters*kilogram<-1>*second<2>)>>>);
}
