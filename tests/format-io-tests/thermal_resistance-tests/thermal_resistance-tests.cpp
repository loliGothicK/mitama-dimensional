...
TEST_CASE("thermal_resistance type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::thermal_resistance)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<-2>*kilogram<-1>*second<3>*kelvins)>>>);
}
