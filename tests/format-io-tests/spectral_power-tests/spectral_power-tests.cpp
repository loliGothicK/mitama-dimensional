...
TEST_CASE("spectral_power type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::spectral_power)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meters*kilograms*second<-3>)>>>);
}
