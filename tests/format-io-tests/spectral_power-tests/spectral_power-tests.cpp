...
TEST_CASE("spectral_power format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::spectral_power_t{}) == "1 [W/m]");
}
TEST_CASE("spectral_power quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::spectral_power) == "1 [W/m]");
}
TEST_CASE("spectral_power type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::spectral_power)>, mitama::quantity_t<std::decay_t<decltype(meters*kilograms*second<-3>)>>>);
}
