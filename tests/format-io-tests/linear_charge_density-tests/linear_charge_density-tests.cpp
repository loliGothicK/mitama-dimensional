...
#include <dimensional/systems/si/derived_units/linear_charge_density.hpp>
...
TEST_CASE("linear_charge_density type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::linear_charge_density)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<-1>*second<>*ampere<>)>>>);
}
