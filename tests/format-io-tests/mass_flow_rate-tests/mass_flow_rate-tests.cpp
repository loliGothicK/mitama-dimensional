...
TEST_CASE("mass_flow_rate type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::mass_flow_rate)>, mitama::systems::si::quantity_t<std::decay_t<decltype(kilograms/seconds)>>>);
}
