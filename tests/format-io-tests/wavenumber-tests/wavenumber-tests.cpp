...
#include <dimensional/systems/si/derived_units/wavenumber.hpp>
...
TEST_CASE("wavenumber type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::wavenumber)>, mitama::quantity_t<std::decay_t<decltype(meter<-1>)>>>);
}
