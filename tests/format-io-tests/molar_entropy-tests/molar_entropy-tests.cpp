...
#include <dimensional/systems/si/derived_units/molar_entropy.hpp>
...
TEST_CASE("molar_entropy type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::molar_entropy)>, mitama::quantity_t<std::decay_t<decltype(meter<2>*kilograms*second<-2>*kelvin<-1>*mol<-1>)>>>);
}
