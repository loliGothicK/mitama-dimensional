...
#include <mitama/dimensional/systems/si/derived_units/energy_flux_density.hpp>
...
TEST_CASE("energy_flux_density type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::energy_flux_density)>, mitama::systems::si::quantity_t<std::decay_t<decltype(kilograms*second<-3>)>>>);
}
