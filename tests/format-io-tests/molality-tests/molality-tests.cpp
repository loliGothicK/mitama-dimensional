...
TEST_CASE("molality format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::molality_t{}) == "1 [mol/kg]");
}
TEST_CASE("molality quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::molality) == "1 [mol/kg]");
}
TEST_CASE("molality type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::molality)>, mitama::quantity_t<std::decay_t<decltype(kilogram<-1>*mol<>)>>>);
}
