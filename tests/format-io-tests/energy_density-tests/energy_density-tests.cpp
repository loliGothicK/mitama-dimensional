...
TEST_CASE("energy_density format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::energy_density_t{}) == "1 [J/m^3]");
}
TEST_CASE("energy_density quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::energy_density) == "1 [J/m^3]");
}
TEST_CASE("energy_density type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::energy_density)>, mitama::quantity_t<std::decay_t<decltype(meter<-1>*kilograms*second<-2>)>>>);
}
