...
#include <mitama/dimensional/systems/si/derived_units/energy_density.hpp>
...
TEST_CASE("energy_density type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::energy_density)>, mitama::quantity_t<std::decay_t<decltype(meter<-1>*kilograms*second<-2>)>>>);
}
