...
#include <mitama/dimensional/systems/si/derived_units/specific_heat_capacity.hpp>
...
TEST_CASE("specific_heat_capacity type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::specific_heat_capacity)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<2>*second<-2>*kelvin<-1>)>>>);
}
