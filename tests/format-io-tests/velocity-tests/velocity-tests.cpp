...
TEST_CASE("velocity format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::velocity_t{}) == "1 [m/s]");
}
TEST_CASE("velocity quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::velocity) == "1 [m/s]");
}
TEST_CASE("velocity type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::velocity)>, mitama::quantity_t<std::decay_t<decltype(meters/seconds)>>>);
}
