...
TEST_CASE("electric_charge_density type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::electric_charge_density)>, mitama::quantity_t<std::decay_t<decltype(meter<-3>*second<>*ampere<>)>>>);
}
