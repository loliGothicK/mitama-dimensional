...
#include <mitama/dimensional/systems/si/derived_units/polarization_density.hpp>
...
TEST_CASE("polarization_density type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::polarization_density)>, mitama::quantity_t<std::decay_t<decltype(meter<-2>*seconds*ampere<>)>>>);
}
