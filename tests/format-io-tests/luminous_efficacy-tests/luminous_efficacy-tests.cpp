...
TEST_CASE("luminous_efficacy type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::luminous_efficacy)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<-2>*kilogram<-1>*second<3>*candelas*steradian)>>>);
}
