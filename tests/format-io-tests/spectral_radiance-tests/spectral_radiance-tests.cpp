...
TEST_CASE("spectral_radiance type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::spectral_radiance)>, mitama::quantity_t<std::decay_t<decltype(meter<-1>*kilograms*second<-3>/steradian)>>>);
}
