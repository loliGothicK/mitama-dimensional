...
TEST_CASE("luminous_energy type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::luminous_energy)>, mitama::systems::si::quantity_t<std::decay_t<decltype(seconds*candelas*steradian)>>>);
}
