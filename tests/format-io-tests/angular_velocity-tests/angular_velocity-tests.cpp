...
#include <mitama/dimensional/systems/si/derived_units/angular_velocity.hpp>
...
TEST_CASE("angular_velocity type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::angular_velocity)>, mitama::quantity_t<std::decay_t<decltype(radian*second<-1>)>>>);
}
