...
TEST_CASE("angular_velocity format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::angular_velocity_t{}) == "1 [rad/s]");
}
TEST_CASE("angular_velocity quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::angular_velocity) == "1 [rad/s]");
}
TEST_CASE("angular_velocity type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::angular_velocity)>, mitama::quantity_t<std::decay_t<decltype(radian*second<-1>)>>>);
}
