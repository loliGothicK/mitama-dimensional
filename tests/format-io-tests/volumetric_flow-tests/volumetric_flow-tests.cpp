...
TEST_CASE("volumetric_flow type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::volumetric_flow)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<3>*second<-1>)>>>);
}
