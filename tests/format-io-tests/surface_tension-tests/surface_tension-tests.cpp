...
TEST_CASE("surface_tension format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::surface_tension_t{}) == "1 [N/m]");
}
TEST_CASE("surface_tension quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::surface_tension) == "1 [N/m]");
}
TEST_CASE("surface_tension type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::surface_tension)>, mitama::quantity_t<std::decay_t<decltype(kilograms*second<-2>)>>>);
}
