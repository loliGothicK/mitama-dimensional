...
TEST_CASE("surface_tension type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::surface_tension)>, mitama::systems::si::quantity_t<std::decay_t<decltype(kilograms*second<-2>)>>>);
}
