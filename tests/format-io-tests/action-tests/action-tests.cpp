...
TEST_CASE("action format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::action_t{}) == "1 [J·s]");
}
TEST_CASE("action quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::action) == "1 [J·s]");
}
TEST_CASE("action type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::action)>, mitama::quantity_t<std::decay_t<decltype(meter<2>*kilograms*second<-1>)>>>);
}
