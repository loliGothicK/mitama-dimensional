...
#include <mitama/dimensional/systems/si/derived_units/acceleration.hpp>
...
TEST_CASE("acceleration type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::acceleration)>, mitama::quantity_t<std::decay_t<decltype(meters/second<2>)>>>);
}
