...
TEST_CASE("specific_angular_momentum type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::specific_angular_momentum)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<2>*second<-1>)>>>);
}
