...
TEST_CASE("moment_of_inertia type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::moment_of_inertia)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<2>*kilograms)>>>);
}
