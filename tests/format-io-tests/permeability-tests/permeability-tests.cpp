...
TEST_CASE("permeability format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::permeability_t{}) == "1 [H/m]");
}
TEST_CASE("permeability quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::permeability) == "1 [H/m]");
}
TEST_CASE("permeability type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::permeability)>, mitama::quantity_t<std::decay_t<decltype(meter<>*kilogram<>*second<-2>*ampere<-2>)>>>);
}
