...
#include <mitama/dimensional/systems/si/derived_units/exposure.hpp>
...
TEST_CASE("exposure type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::exposure)>, mitama::quantity_t<std::decay_t<decltype(kilogram<-1>*second<>*ampere<>)>>>);
}
