...
#include <dimensional/systems/si/derived_units/molar_energy.hpp>
...
TEST_CASE("molar_energy type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::molar_energy)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<2>*kilograms*second<-2>*mol<-1>)>>>);
}
