...
#include <mitama/dimensional/systems/si/derived_units/specific_volume.hpp>
...
TEST_CASE("specific_volume type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::specific_volume)>, mitama::quantity_t<std::decay_t<decltype(meter<3>*kilogram<-1>)>>>);
}
