...
TEST_CASE("thermal_conductivity type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::thermal_conductivity)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<>*kilograms*second<-3>*kelvin<-1>)>>>);
}
