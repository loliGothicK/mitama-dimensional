...
TEST_CASE("magnetic_reluctance format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::magnetic_reluctance_t{}) == "1 [H^-1]");
}
TEST_CASE("magnetic_reluctance quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::magnetic_reluctance) == "1 [H^-1]");
}
TEST_CASE("magnetic_reluctance type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::magnetic_reluctance)>, mitama::quantity_t<std::decay_t<decltype(meter<-2>*kilogram<-1>*second<2>*ampere<2>)>>>);
}
