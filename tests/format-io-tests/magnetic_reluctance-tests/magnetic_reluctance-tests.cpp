...
#include <dimensional/systems/si/derived_units/magnetic_reluctance.hpp>
...
TEST_CASE("magnetic_reluctance type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::magnetic_reluctance)>, mitama::quantity_t<std::decay_t<decltype(meter<-2>*kilogram<-1>*second<2>*ampere<2>)>>>);
}
