...
#include <mitama/dimensional/systems/si/derived_units/kinematic_viscosity.hpp>
...
TEST_CASE("kinematic_viscosity type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::kinematic_viscosity)>, mitama::quantity_t<std::decay_t<decltype(meter<2>*second<-1>)>>>);
}
