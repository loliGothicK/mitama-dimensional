...
TEST_CASE("magnetic_susceptibility type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::magnetic_susceptibility)>, mitama::quantity_t<std::decay_t<decltype(meter<-1>*kilogram<-1>*second<2>*ampere<2>)>>>);
}
