...
TEST_CASE("spectral_irradiance format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::spectral_irradiance_t{}) == "1 [W/m^3]");
}
TEST_CASE("spectral_irradiance quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::spectral_irradiance) == "1 [W/m^3]");
}
TEST_CASE("spectral_irradiance type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::spectral_irradiance)>, mitama::quantity_t<std::decay_t<decltype(meter<-1>*kilograms*second<-3>)>>>);
}
