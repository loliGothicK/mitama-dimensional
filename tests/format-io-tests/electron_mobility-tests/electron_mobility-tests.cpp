...
TEST_CASE("electron_mobility type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::electron_mobility)>, mitama::systems::si::quantity_t<std::decay_t<decltype(kilogram<-1>*second<2>*ampere<1>)>>>);
}
