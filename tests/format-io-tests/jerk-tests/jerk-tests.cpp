...
TEST_CASE("jerk format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::jerk_t{}) == "1 [m/s^3]");
}
TEST_CASE("jerk quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::jerk) == "1 [m/s^3]");
}
TEST_CASE("jerk type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::jerk)>, mitama::quantity_t<std::decay_t<decltype(meters/second<3>)>>>);
}
