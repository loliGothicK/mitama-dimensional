...
TEST_CASE("magnetic_field_strength format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::magnetic_field_strength_t{}) == "1 [A/m]");
}
TEST_CASE("magnetic_field_strength quantifier format test", "[quantity][abbreviation]") {
  REQUIRE(fmt(1 | si::magnetic_field_strength) == "1 [A/m]");
}
TEST_CASE("magnetic_field_strength type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|si::magnetic_field_strength)>, mitama::quantity_t<std::decay_t<decltype(meter<-1>*ampere<>)>>>);
}
