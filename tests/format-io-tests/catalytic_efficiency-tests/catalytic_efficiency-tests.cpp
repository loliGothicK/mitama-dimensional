...
TEST_CASE("catalytic_efficiency type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::catalytic_efficiency)>, mitama::quantity_t<std::decay_t<decltype(meter<3>*second<-1>*mol<-1>)>>>);
}
