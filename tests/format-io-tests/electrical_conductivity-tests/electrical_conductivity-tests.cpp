...
#include <mitama/dimensional/systems/si/derived_units/electrical_conductivity.hpp>
...
TEST_CASE("electrical_conductivity type test", "[quantity][abbreviation]") {
  REQUIRE(mitama::is_same_dimensional_v<std::decay_t<decltype(1|systems::si::electrical_conductivity)>, mitama::systems::si::quantity_t<std::decay_t<decltype(meter<-3>*kilogram<-1>*second<3>*ampere<2>)>>>);
}
