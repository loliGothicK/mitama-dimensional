...
template <> struct abbreviation<meter_t> { static constexpr char str[] = "m"; };
template <> struct abbreviation<millimeter_t> {
  static constexpr char str[] = "mm";
};
template <> struct abbreviation<centimeter_t> {
  static constexpr char str[] = "cm";
};
template <> struct abbreviation<nanometer_t> {
  static constexpr char str[] = "nm";
};
template <> struct abbreviation<second_t> {
  static constexpr char str[] = "s";
};
template <> struct abbreviation<millisecond_t> {
  static constexpr char str[] = "ms";
};
template <> struct abbreviation<nanosecond_t> {
...
operator<<(std::ostream &os,
  ...
  } else if constexpr (std::conjunction_v<is_complete_type<abbreviation<
                           dimensional_t<_, typename Units::basic_type>>>...>) {
    os << quantity.get() << "[ ";
    ((os << abbreviation<dimensional_t<_, typename Units::basic_type>>::str
         << exponent<typename Units::exponent> << " "),
     ...);
    return os << "]";
  } else {
