...
namespace mitama::mitamagic {
    ...
    struct tlist_cat<Pack<Elems...>>
        { using type = Pack<Elems...>; }; // List concatenate: binary
    ...
    template <template <class> class Pred, template <class...> class tPack,
              class... Args>
    struct tlist_all_match_if<Pred, tPack<Args...>> : std::conjunction<Pred<Args>...> {};

    template < template <class> class Pred, class TypeList >
    inline constexpr bool tlist_all_match_if_v = tlist_all_match_if<Pred, TypeList>::value;

    template < template<class...>class, class > struct repack;

    template < template<class...>class Pack, template <class...> class _, class... Elems >
    struct repack<Pack, _<Elems...>> { using type = Pack<Elems...>; };
